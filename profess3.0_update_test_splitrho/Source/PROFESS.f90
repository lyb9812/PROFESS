...
!------------------------------------------------------------------------------
  ...
  USE Sys, ONLY : TDDFT_flag  ! 03/08/2021 ---- Yuanbo Li    
  ...
  ! 2) MAIN ROUTINES 
  !---------------------------
  ! add TDDFT moudle 03/08/2021 ---- Yuanbo Li
  IF (TDDFT_flag .eqv. .FALSE.) THEN    
    IF (mdType == -1) THEN
      IF (rankGlobal==0) THEN 
  ...
  ELSE
    IF (TDDFT_flag .eqv. .TRUE.) THEN 
      WRITE(*,*) " start Time-dependent dft calculations "
      IF (rankGlobal==0) PRINT *, ' >> Calling TDDFT Propagate() ...'
      CALL Propagate
    ENDIF
  ENDIF
  !---------------------------
  ! 3) FINISH ALL 
  !---------------------------
  IF (TDDFT_flag .eqv. .FALSE.) CALL FinalizeOutput  ! 3/9/2021 ---- Yuanbo Li
