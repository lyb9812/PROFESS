...
!!!!! BELOW THIS LINE ARE PARAMETERS DIRECTLY MODIFIED BY THIS MODULE !!!!!
  ...
  USE Sys,ONLY: TDDFT_flag  ! 02/27/2021 Yuanbo Li 
  ...
  !USE TDDFT, ONLY : rho_in
  !USE TDDFT, ONLY : current_in
  USE TDDFT, ONLY : Hamiltonian_version
  USE TDDFT, ONLY : propagation_method
  USE TDDFT, ONLY : ion_method  
...
!--------------------------------------------------------------------------
  ...
  !
  ! CHARACTER(LEN=40) :: tempChar
  ! temporary character strings  ---- 03/06/2021 Yuanbo Li
  ...
  DO
    ...
    SELECT CASE (TRIM(keyword(1:4)))
     ...
     !-----------------------------------------------------------------------------
     !52) TDDFT parameters   03/06/2021 Yuanbo Li
     CASE("TDDF")
       !READ(inputUnit,*,IOSTAT=fileStatus) keyword,t_dt,t_temperature,t_Nstep,rho_in,current_in,version,propagation_method,ion_method
       !TDDFT_flag = .true. 
       !CALL WrtOut(6, " (input) Perform Time-dependent dft calculations ")
       !WRITE(*,*) 'tddft parameters: ',t_dt,t_temperature,t_Nstep,TRIM(rho_in),TRIM(current_in),version,propagation_method,ion_method 
       READ(inputUnit,*,IOSTAT=fileStatus) keyword, option, tempReal
       TDDFT_flag = .true.
       outputFinalGeometry = .TRUE.
       IF (fileStatus/=0) THEN
          WRITE(message,*) 'TDDFT format error. Format is:TDDFT [OPTION] [REALVALUE].'
          CALL Error(6, message)
       ENDIF
       !CALL WrtOut(6, " (input) Perform Time-dependent dft calculations ")
       CALL Uppercase(option)
       SELECT CASE (TRIM(option(1:4)))
         CASE("TEMP")     ! temperature
           t_temperature = tempReal
           WRITE(*,*) "TDDFT calculation temperature : ",t_temperature 
         CASE("DTIM")  ! time interval
           t_dt = tempReal / fundamentalTime
           WRITE(*,*) "TDDFT calculation time interval : ", t_dt
         CASE("NSTE")  ! number of total steps
           t_Nstep = tempReal
           WRITE(*,*) "TDDFT calculation number of steps : ", t_Nstep
         CASE("HAMI")  ! ! the version of the form of Hamiltonian
                       !    0 : H=-\nabla^2/2+v_TF+v_Hartree+v_ext+v_xc
                       !    1 : H=-\nabla^2/2+v_TF+v_Hartree+v_ext+v_xc+v_CD
           Hamiltonian_version = INT(tempReal)
           WRITE(*,*) "TDDFT calculation version of Hamiltonian : ", Hamiltonian_version
         CASE("PROP")  ! the method to propagate the Madelung wave function
                       !    0 : Runge-Kutta method 
                       !    1 : Crank-Nicolson method 
                       !    2 : Split operator method
           propagation_method = INT(tempReal)
           WRITE(*,*) "TDDFT calculation propagation method of the Madelung wavefunction : ", propagation_method
         CASE("IONM")  ! the method to move the ions
                       !    0 : Erenfest dynamics
           ion_method = INT(tempReal)
           WRITE(*,*) "TDDFT calculation method of ionic movements : ", ion_method
         CASE("XCFU")  ! type of xc functional
                       !    0 : LDA
                       !    1 : PBE
           xc_funct = INT(tempReal)
       END SELECT
