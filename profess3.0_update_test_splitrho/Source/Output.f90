...
!------------------------------------------------------------------------------
  ...
  velocity,&          ! velocity of ions 3/9/2021 Yuanbo Li
  forces              ! forces of ions 3/9/2021 Yuanbo Li
  USE SYS, ONLY : psi  ! test 5/4/2021 Yuanbo Li
 
  USE TDDFT, ONLY : TDE 
  USE TDDFT, ONLY : TDE0
  ! time dependent total energy 4/16/2021 Yuanbo Li
  USE TDDFT, ONLY : IonKinetic 
  ! the total kinetic energy of the ions 4/16/2021 Yuanbo Li
  USE TDDFT, ONLY : t_time
  ! the current time 5/4/2021 Yuanbo Li
  USE TDDFT, ONLY : totalcharge
  ! for testing 5/6/2021 Yuanbo Li
  ...
  USE MATHFUNCTIONS, ONLY : Vecmul  ! 3/9/2021 Yuanbo Li
  ...
  TDXUnit = 17, &          ! TDDFT cartesian coordinates of ions  3/9/2021 Yuanbo Li
  TDVUnit = 18, &          ! TDDFT  velocitys of ions  3/9/2021 Yuanbo Li
  TDFUnit = 19, &          ! TDDFT  forces of ions  3/9/2021 Yuanbo Li
  TDEUnit = 20, &          ! TDDFT  energies 4/12/2021 Yuanbo Li
...
!------------------------------------------------------------------------------
  ...
  ! for testing Yuanbo Li
  !WRITE(*,*) "parallel rhoR size : ", lbound(rhoR,3),ubound(rhoR,3)
  zn = 0
  DO isp = 0, mysize-1
    IF(myrank == isp) THEN
...
SUBROUTINE Print_TDDFT
  IF ( outputRank==0 ) THEN   ! 3/25/2021 Yuanbo Li
    !CALL PrintTDX
    !CALL PrintTDV
    !CALL PrintTDF
    CALL PrintTDE
    ! test
    !OPEN(unit=300, access="sequential", action="readwrite", &
    !   blank="null", delim="none", file="testrho.txt", form="formatted", &
    !    pad="no", position="append", status="unknown")
    !WRITE(300,*) t_time*0.024189_DP, ABS(psi(1,1,1))**2, ABS(psi(2,2,2))**2 

    !OPEN(unit=301, access="sequential", action="readwrite", &
    !   blank="null", delim="none", file="totalcharge.txt", form="formatted", &
    !    pad="no", position="append", status="unknown")
    !WRITE(301,*) t_time*0.024189_DP, totalcharge
  END IF
  RETURN
END SUBROUTINE Print_TDDFT

SUBROUTINE PrintTDX
! print cartesian coordinates of ions
  IMPLICIT NONE
  INTEGER(KIND=DP) :: i
  INTEGER :: fileStatus
  CHARACTER(LEN=50) :: TDXFile
  REAL(KIND=DP), DIMENSION(3) :: TDX

  TDXFile = TRIM(outputSystemName) // ".TDX"
  
  OPEN(unit=TDXUnit, access="sequential", action="readwrite", &
       blank="null", delim="none", file=TRIM(TDXFile), form="formatted", &
       iostat=fileStatus, pad="no", position="append", status="unknown")  

  ! Format Descriptors
  10 FORMAT(A)
  11 FORMAT(I6, 3F20.15)
   
  IF (fileStatus/=0) THEN
    WRITE(*,*)'The file ',TRIM(TDXFile),' had a problem on opening.'
    WRITE(*,*)'And I cannot even begin to imagine what it might be.'
    STOP
  ELSE
    WRITE(TDXUnit,10) " "
    DO i=1, SIZE(cell%ionTable)
       TDX = Vecmul(cell%cellReal,cell%ionTable(i)%coord)
       WRITE(TDXUnit,11) i, TDX(1)*bohr, TDX(2)*bohr, TDX(3)*bohr 
       ! convert to  angstrom
    END DO
  END IF

END SUBROUTINE PrintTDX

SUBROUTINE PrintTDV
! print velocitys of ions
  IMPLICIT NONE
  INTEGER(KIND=DP) :: i
  INTEGER :: fileStatus
  CHARACTER(LEN=50) :: TDVFile

  TDVFile = TRIM(outputSystemName) // ".TDV"

  OPEN(unit=TDVUnit, access="sequential", action="readwrite", &
       blank="null", delim="none", file=TRIM(TDVFile), form="formatted", &
       iostat=fileStatus, pad="no", position="append", status="unknown")
  ...
  IF (fileStatus/=0) THEN
    WRITE(*,*)'The file ',TRIM(TDVFile),' had a problem on opening.'
    WRITE(*,*)'And I cannot even begin to imagine what it might be.'
    STOP
  ELSE
    WRITE(TDVUnit,10) " "
    DO i=1, SIZE(cell%ionTable)
       WRITE(TDVUnit,11) i, velocity(1,i)*bohr/(1.0E15_DP * fundamentalTime), &
                            velocity(2,i)*bohr/(1.0E15_DP * fundamentalTime), &
                            velocity(3,i)*bohr/(1.0E15_DP * fundamentalTime)
        ! convert to angstorm/fs
    END DO
  END IF

END SUBROUTINE PrintTDV
...
! print velocitys of ions
  ...
  INTEGER(KIND=DP) :: i
  INTEGER :: fileStatus
  CHARACTER(LEN=50) :: TDFFile

  TDFFile = TRIM(outputSystemName) // ".TDF"

  OPEN(unit=TDFUnit, access="sequential", action="readwrite", &
       blank="null", delim="none", file=TRIM(TDFFile), form="formatted", &
       iostat=fileStatus, pad="no", position="append", status="unknown")
  ...
  IF (fileStatus/=0) THEN
    WRITE(*,*)'The file ',TRIM(TDFFile),' had a problem on opening.'
    WRITE(*,*)'And I cannot even begin to imagine what it might be.'
    STOP
  ELSE
    WRITE(TDFUnit,10) " "
    DO i=1, SIZE(cell%ionTable)
       WRITE(TDFUnit,11) i, forces(i,1,1), forces(i,2,1), forces(i,3,1)
    END DO
  END IF

END SUBROUTINE PrintTDF
...
! print energyies
  ...
  INTEGER(KIND=DP) :: i
  INTEGER :: fileStatus
  CHARACTER(LEN=50) :: TDEFile

  TDEFile = TRIM(outputSystemName) // ".TDE"

  OPEN(unit=TDEUnit, access="sequential", action="readwrite", &
       blank="null", delim="none", file=TRIM(TDEFile), form="formatted", &
       iostat=fileStatus, pad="no", position="append", status="unknown")

  ! Format Descriptors
  10 FORMAT(A12, " =", 1X, ES20.12, " eV")

  IF (fileStatus/=0) THEN
    WRITE(*,*)'The file ',TRIM(TDEFile),' had a problem on opening.'
    WRITE(*,*)'And I cannot even begin to imagine what it might be.'
    STOP
  ELSE
    WRITE(*,*) "test each energy",energy
    !WRITE(TDEUnit,*) "TOTAL ENERGY", TDE*hartreeToeV, IonKinetic*hartreeToeV
    WRITE(TDEUnit,*) t_time*0.024189_DP , TDE0*hartreeToeV,IonKinetic*hartreeToeV!, &
    ! energy(2)*hartreeToeV, &
    ! energy(3)*hartreeToeV, &
    ! energy(4)*hartreeToeV, &
    !energy(5)*hartreeToeV, &
    ! energy(6)*hartreeToeV
  END IF

END SUBROUTINE PrintTDE
