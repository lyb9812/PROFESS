...
!----------------------------------------------------------------------------
  USE Output, ONLY : Wrtout
  USE Constants, ONLY : bohr
  USE SYS, ONLY: forceIon  ! Mohan added 2019-01-26
  USE Output, ONLY: outputIonTable ! Mohan added 2019-04-01

  IMPLICIT NONE
  LOGICAL :: pass
  ! mohan added uf2 for output of forces, 2019-01-26
  INTEGER  :: i,iter,uf, uf2
  CHARACTER(LEN=100) :: iteration_number
  ! mohan added message2 for output of forces , 2019-01-26
  CHARACTER(LEN=500) :: message, message2, eleName
  ...
  uf = 1999
  uf2 = 19992 ! mohan added, 2019-01-26
  ...
  WRITE(message2,'(a,a,a,a)') TRIM(md_output_path),'/for.',TRIM(ADJUSTL(iteration_number)),'.dat'
  ...
  ! mohan added 2019-01-26
  !-------------------------------
  ! open the file: 'for.1.dat'
  ! and write force data
  !-------------------------------
  OPEN (UNIT=uf2, FILE=message2, ACTION='write')
  DO i = 1, cell%numIon
    WRITE(eleName ,'(a)') &
      cell%elementTable(cell%ionTable(i)%elementID)%elementName
    WRITE(message,'(a,a,3(a,Es16.8))') &
      "  ",TRIM(ADJUSTL(eleName)), &
      "  ",forceIon(i,1,1)*hartreeToeV/bohr, & ! unit
      "  ",forceIon(i,2,1)*hartreeToeV/bohr, &
      "  ",forceIon(i,3,1)*hartreeToeV/bohr
    CALL WrtOut(uf2,message)
  ENDDO
  
  !-------------------------------------------------------
  ! Print the atomic coordinates in Cartesian coordinates
  !-------------------------------------------------------
  ...
  ! mohan added 2019-04-01
  !-------------------------------------------------------
  ! Print the atomic coordinates in Direct coordinates
  !-------------------------------------------------------
  WRITE(message,'(a)') '%BLOCK POSITIONS_FRAC'
  CALL WrtOut(uf,message)
  DO i = 1, cell%numIon
    WRITE(eleName ,'(a)') & 
      cell%elementTable(cell%ionTable(i)%elementID)%elementName
    ...
    WRITE(message,'(a,a,3(a,Es16.8))') & 
      "  ",TRIM(ADJUSTL(eleName)), & 
      "  ",cell%ionTable(outputIonTable(i))%coord(1), &
      "  ",cell%ionTable(outputIonTable(i))%coord(2), &
      "  ",cell%ionTable(outputIonTable(i))%coord(3)
    CALL WrtOut(uf,message)
  ENDDO
  WRITE(message,'(A)') '%END BLOCK POSITIONS_FRAC'
  CALL WrtOut(uf,message)
